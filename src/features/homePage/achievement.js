...
export default function initAchievement() {
  ctx = gsap.context(() => {
    ...
    const headlineSplit = new SplitType(achievementHeadline, { types: 'chars' })
    ...
    achievementElements.each((index, element) => {
      ...
      achievementTl
        .from(valueSplit.chars, { y: 125, duration: 0.5, stagger: 0.01 }, '<+0.25')
        .from(typeSplit.chars, { y: 125, duration: 0.5, stagger: 0.001 }, '<')
        .from(unitSplit.chars, { y: 125, duration: 0.5, stagger: 0.001 }, '<')
        .to(unitSplit.chars, { y: -125, duration: 0.5, stagger: 0.001 }, '>+0.1')
        .to(valueSplit.chars, { y: -125, duration: 0.5, stagger: 0.01 }, '<')
        .to(typeSplit.chars, { y: -125, duration: 0.5, stagger: 0.001 }, '<')
    })
